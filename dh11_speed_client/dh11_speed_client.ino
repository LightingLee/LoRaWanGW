...
void InitDHT()
{
    ...
    pinMode(speed_pin,INPUT);//Set A2 to output
...
void loop()
{
    ...
    if(ReadSpeed())
    {
      ...
      if(pulse == 334)
      {
        counter++;
        pulse = 0;
        //Serial.print("One roop!, now counter is ");
        //Serial.println(counter);//Show speed
      }
    }

    if(nowtime10 - lasttime10 > 5000){

      lasttime10 = nowtime10; 
      sendBuf1[0] = 1;
      sendBuf1[1] = (char)(counter/5);
      counter = 0;

      Serial.print("Speed is ");
      Serial.print(sendBuf1[1], DEC);//Show speed
      Serial.println("/sec");

      rf95.send(sendBuf1, 2);//Send LoRa Data
    }

    if(nowtime30 - lasttime30 > 30000){      
      lasttime30 = nowtime30;
      ReadDHT();
      char data[50] = {0} ;
      // Use data[0] as Node ID
      data[0] = 2 ;
      data[1] = dht_dat[0];//Get Humidity
      data[2] = dht_dat[2];//Get Temperature
      ...
      switch (bGlobalErr)
      {
        case 0:
            ...
            Serial.print(data[2], DEC);//Show temperature
            Serial.print(".");
    ...
    rf95.send(sendBuf2, 3);//Send LoRa Data
  }
}